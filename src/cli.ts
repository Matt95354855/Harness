...
import { createConfiguredToolRuntime } from './tools/configured-runtime.js';
...
async function main(): Promise<void> {
  ...
  if (values.version) { console.log('0.2.0'); return; }
  ...
  if (command === 'doctor') {
    ...
    try {
      const report: Record<string, unknown> = { ok: true, provider: config.provider, model: config.agent.model, search: config.search.provider, capabilities: runtime.capabilities, tools: runtime.registry.listAll().map(tool => tool.name), node: process.version };
  ...
  const runtime = command === 'demo' ? undefined : await createConfiguredToolRuntime();
  const agent = command === 'demo' ? createDemoAgent() : new Agent({}, { registry: runtime!.registry });
  ...
  } finally { process.removeListener('SIGINT', interrupt); await runtime?.close(); }
}
